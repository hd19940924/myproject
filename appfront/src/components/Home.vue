<template>
  <div class="home">
    <el-row display="margin-top:10px" >
        ...
        <el-button type="primary" @click="showBooks()" style="float:left; margin: 2px;">查询</el-button>
    </el-row>
    <el-row>
...
export default {
  ...
  methods: {
    ...
    showBooks () {
      this.$http.get('http://127.0.0.1:8000/show_books?book_name=' + this.input)
        .then((response) => {
          var res = JSON.parse(response.bodyText)
          console.log(res)
          if (res.error_num === 0) {
            this.bookList1 = res['list']
            this.bookList=this.bookList1.slice(
               (this.page - 1) * this.size,
               this.page * this.size
           );
            this.total=this.bookList1.length
          }else {
